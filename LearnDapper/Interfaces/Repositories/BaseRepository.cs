...
namespace LearnDapper.Interfaces.Repositories
{
    public class BaseRepository<TEntity> : IRepository<TEntity>, IReadRepository<TEntity> where TEntity : class
    {
        ...
        {
            _context.Set<TEntity>().AddRange(entities);
            return await _context.SaveSchangesAsync(cancellationToken);
        }

        public virtual IQueryable<TEntity> GetAll(bool asNoTrcking = true)
        {
            if(asNoTrcking)
        ...
        public virtual IQueryable<TEntity> GetAllIncluding(params Expression<Func<TEntity, object>>[] includiProperties)
        {
            IQueryable<TEntity> querable = GetAll();
            foreach (Expression<Func<TEntity, object>> incldudeProperty in includiProperties)
            {
                querable = querable.Include(incldudeProperty);
            }
            return querable;
        }
        ...
        public virtual IQueryable<TEntity> GetBySpecs(Expression<Func<TEntity, bool>> predicate, bool asNoTracking = true)
        {
            if (asNoTracking)
        ...
        public virtual Task<TEntity?> GetBySpecsAsync<Spec>(Expression<Func<TEntity, bool>> predicate, CancellationToken cancellationToken = default)
        {
            return _context.Set<TEntity>().FirstOrDefaultAsync(predicate, cancellationToken: cancellationToken);
        }
        ...
        {
            return await _context.Set<TEntity>().Where(predicate).ToListAsync(cancellationToken: cancellationToken);
